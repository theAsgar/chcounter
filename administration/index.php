...
switch( $_GET['cat'] )
{
	...
	// by ib
   case 'bots':                $title = ' - '. $_CHC_LANG['logs'].'-Robots'; break;
  // end ib
...
?>
    [<a href="index.php?cat=logs"><?php print $_CHC_LANG['logs']; ?></a>]&nbsp;
     <!-- // added by ib Zuletzt geändert am 13.02.2010-->
<?php

if( !isset( $_GET['gruppe'] ) )
{
        $_GET['gruppe'] = 'timestamp';
}
if( !isset( $_GET['mysort'] ) )
{
        $_GET['mysort'] = $_CHC_CONFIG['anordnung_log_eintraege'] ;
}
?>

     [<a href="index.php?cat=bots&gruppe=<?php echo $_GET['gruppe'];?>"><?php print $_CHC_LANG['logs'].'-Robots'; ?></a>] 
   <!-- // end by ib -->
...
else
{
	...
	if( $_GET['cat'] != 'settings' )
	{
		...
		// In nächster Zeile ergänzt by ib: || $_GET['cat'] == 'bots' (wegen gleicher Formatierung)
		if( $_GET['cat'] == 'logs' || $_GET['cat'] == 'downloads' || $_GET['cat'] == 'hyperlinks'  || $_GET['cat'] == 'bots')
		{
	...
	switch( $_GET['cat'] )
	{
		...
		// added by ib
   case 'bots':               require( 'logs_bots.inc.php' ); break;
   // end by ib
