...
else
{
  ...
  if( version_compare( $_CHC_CONFIG['script_version'], '3.1.2', '<' ) == 1 )
	{
		$_CHC_DB->query(	'ALTER TABLE `'. CHC_TABLE_DOWNLOADS_AND_HYPERLINKS .'` add `monat` int(1) NOT NULL ;' );
		chC_set_config( 'script_version', '3.1.2' );
	}

  if( version_compare( $_CHC_CONFIG['script_version'], '3.1.3', '<' ) == 1 )
	{
		chC_set_config( 'anordnung_log_eintraege', 'DESC' );
		chC_set_config( 'statistiken_anzahl_referrer', '20' );
		chC_set_config( 'statistiken_anzahl_refdomains', '20' );
		chC_set_config( 'statistiken_anzahl_user_agents', '20' );
		chC_set_config( 'counterstatus_statistikseiten', '0' );
		chC_set_config( 'statistiken_login_erforderlich', 'index; index:main; index:pages; index:downloads_and_hyperlinks; index:referrers; index:visitors_details; index:access_statistics; index:all_lists; online_users; versions; ' );
		chC_execute_sql_file( './mysql/delete_spammer.sql' );
		chC_execute_sql_file( './mysql/blacklist.sql' );
		chC_set_config( 'script_version', '3.1.3' );
	}
